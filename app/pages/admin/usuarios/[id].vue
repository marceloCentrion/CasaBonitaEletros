...
export default {
  ...
  setup() {
    ...
    async function fetchUsuario() {
      try {
        const { data } = await services.usuarios.getById({
    ...
    async function salvarUsuario() {
      ...
      dados.id = state.id;
      try {
        await services.usuarios.update(dados, token);
        toast.success("Usuário atualizado com sucesso!");
        router.push('/admin/usuarios');
      } catch (error) {
        console.log(error);
        toast.error(error.response?.data?.msg || error.response?.data?.message || "Erro ao atualizar usuário.");
      } finally {
