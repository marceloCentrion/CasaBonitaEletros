              ...
              <div class="row">
                ...
                <div class="col-2">
                  <div style="text-align: right">
                    <NuxtLink class="no-link" to="/admin/usuarios/cadastrar-usuario">
                      <AdminButton>
                        Novo Usuário
                      </AdminButton>
                    </NuxtLink>
            ...
            <div class="table-responsive">
              ...
              <div class="card-body" v-else>
                <table class="table table-hover">
                  <thead>
                    <tr>
                      <th style="background-color: var(--primary); color: #fff" scope="col">
                        Código
                      </th>
                      <th style="background-color: var(--primary); color: #fff" scope="col">
                        Nome
                  ...
                  <tbody>
                    <tr v-for="usuario in state.usuarios" :key="usuario.id">
                      ...
                      <td>
                        ...
                        <button
                          @click="abrirModalDeletar(usuario.id, usuario.nome)"
                          title="Deletar"
        ...
        <Transition name="modal-slide">
          <div v-if="modalDeletar" class="modal-confirm" role="dialog" aria-modal="true" aria-labelledby="modal-titulo">
            ...
            <div class="modal-confirm__body">
              ...
              <strong>{{ usuarioNomeParaDeletar }}</strong>
            ...
            <div class="modal-confirm__footer">
              ...
              <button class="btn-modal btn-modal--confirmar" @click="deletarUsuario" :disabled="isLoadingStore.isLoading">
...
export default {
  setup() {
    useHead({
      title: "Casa Bonita Eletros - Usuários",
    });
    ...
    const usuarioIdParaDeletar = ref(null);
    const usuarioNomeParaDeletar = ref("");
    ...
    function abrirModalDeletar(id, nome) {
      usuarioIdParaDeletar.value = id;
      usuarioNomeParaDeletar.value = nome;
      modalDeletar.value = true;
    }

    function fecharModalDeletar() {
      modalDeletar.value = false;
      usuarioIdParaDeletar.value = null;
      usuarioNomeParaDeletar.value = "";
    }
    ...
    async function deletarUsuario() {
      const id = usuarioIdParaDeletar.value;
      if (!id) return;

      try {
        isLoadingStore.start();
        const { data } = await services.usuarios.delete({ id, token });
        fecharModalDeletar();
        
        if (data && data.status && data.status != 200 && data.status != 201) {
          modalErro.value.mensagem = data.message || data.msg || "Erro ao deletar usuário.";
        ...
        if (errorMessage.toLowerCase().includes("constraint") || error.response?.status === 400 || error.response?.status === 500) {
          modalErro.value.mensagem = "Não é possível excluir este usuário pois existem registros vinculados a ele.";
        } else {
          modalErro.value.mensagem = errorMessage || "Ocorreu um erro inesperado ao tentar excluir o usuário. Tente novamente mais tarde.";
        }
        modalErro.value.show = true;
      } finally {
        isLoadingStore.stop();
...
/* Estilos dos Modais (Mesmo padrão de Produtos) */
