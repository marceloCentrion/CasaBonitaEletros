              ...
              <div class="row">
                ...
                <div class="col-2">
                    <div style="text-align: right">
                      <NuxtLink
                        class="no-link"
                        to="/admin/variacoes/cadastrar-variacao"
                      >
                        <AdminButton> Nova Variação </AdminButton>
                      </NuxtLink>
                    </div>
                </div>
              </div>
                  ...
                  <tbody>
                    <tr v-for="vari in state.variacoes" :key="vari.id">
                      ...
                      <td>
                        ...
                        <button
                          @click="abrirModalDeletar(vari.id, vari.nome)"
                          title="Deletar"
      ...
      <Transition name="modal-slide">
        ...
        >
          ...
          <div class="modal-confirm__body">
            <p>Tem certeza que deseja deletar a variação:</p>
            <strong>{{ state.variacaoNomeParaDeletar }}</strong>
...
export default {
  setup() {
    useHead({
      title: "Casa Bonita Eletros - Variações",
    });
    ...
    const state = reactive({
      ...
      modalDeletar: false,
      variacaoIdParaDeletar: null,
      variacaoNomeParaDeletar: "",
    });
    onMounted(() => {
      fetchVariacoes();
    });
    async function fetchVariacoes() {
      try {
        const { data } = await services.variacoes.getAll({ token: authStore.token });
    ...
    function abrirModalDeletar(id, nome) {
      state.variacaoIdParaDeletar = id;
      state.variacaoNomeParaDeletar = nome;
      state.modalDeletar = true;
    }

    function fecharModalDeletar() {
      state.modalDeletar = false;
      state.variacaoIdParaDeletar = null;
      state.variacaoNomeParaDeletar = "";
    }
    ...
    async function deletarVariacao() {
      const id = state.variacaoIdParaDeletar;
      if (!id) return;
      ...
      try {
        ...
        if (data && data.status && data.status != 200 && data.status != 201) {
          state.modalErro.mensagem = data.message || data.msg || "Erro ao deletar a variação.";
        ...
        if (errorMessage.toLowerCase().includes("constraint") || error.response?.status === 400 || error.response?.status === 500) {
          state.modalErro.mensagem = "Não é possível excluir esta variação pois existem produtos vinculados a ela. Por favor, remova as variações dos produtos associados antes de prosseguir.";
        } else {
          state.modalErro.mensagem = "Ocorreu um erro inesperado ao tentar excluir a variação. Tente novamente mais tarde.";
        }
        state.modalErro.show = true;
      } finally {
