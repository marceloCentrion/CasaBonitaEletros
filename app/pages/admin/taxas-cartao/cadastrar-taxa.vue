    ...
    <div class="card">
      ...
      <div class="card-body">
        ...
        <div class="mt-5 pt-3 d-flex justify-content-end gap-3">
          ...
          <SecButton class="w-fit-content px-8" @click="salvarTaxa" :disabled="state.isLoading">
...
export default {
  ...
  setup() {
    useHead({
      title: "Casa Bonita Eletros - Cadastrar taxa",
    });
    ...
    async function salvarTaxa() {
      ...
      try {
        await services.taxas.save(dados, token);
        toast.success("Taxa cadastrada com sucesso!");
        router.push("/admin/taxas-cartao");
      } catch (error) {
        console.log(error);
        toast.error(error.response?.data?.msg || error.response?.data?.message || "Erro ao cadastrar taxa.");
      } finally {
